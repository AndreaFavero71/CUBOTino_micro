...
class Settings:
    ...
    def check_local_settings(self):
        ...
        fnames_default = ('Cubotino_m_settings_default.txt' , 'Cubotino_m_servo_settings_default.txt')
        fnames_local = ('Cubotino_m_settings.txt', 'Cubotino_m_servo_settings.txt')
    ...
    def get_settings_fname(self):
        fname = 'Cubotino_m_settings.txt'                         # file name with general settings
        if self.eth_mac in self.macs_AF:                          # case the script is running on AF (Andrea Favero) robot
            pos = self.macs_AF.index(self.eth_mac)                # mac address row position in macs_AF.txt file
    ...
    def get_servo_settings_fname(self):
        fname = 'Cubotino_m_servo_settings.txt'                   # file name with servos settings
        if self.eth_mac in self.macs_AF:                          # case the script is running on AF (Andrea Favero) robot
            pos = self.macs_AF.index(self.eth_mac)                # mac address row position in macs_AF.txt file
            fname = self.get_fname_AF(fname, pos)                 # AF robot s file name (do not use these at the start)
        else:                                                     # case the script is not running on AF (Andrea Favero) robot
    ...
    def read_settings(self, fname=''):
        """Function reading the Cubotino_m_settings.txt file. Retrieves a dict with parameters and settings."""
        ...
        if os.path.exists(fname):                                 # case the settings file exists
            ...
            backup_fname = 'Cubotino_m_settings_backup.txt'       # filename for the backup
    ...
    def parse_settings(self, s):
        ...
        try:
            ...
            s['camera_height_res'] = int(s['camera_height_res'])  # Picamera resolution on heigh
    ...
    def read_servos_settings(self, fname=''):
        """Function reading the Cubotino_m_servo_settings.txt . Retrieves a dict with parameters and settings."""
        ...
        if os.path.exists(fname):                                 # case the servo_settings file exists
            ...
            backup_fname = 'Cubotino_m_servo_settings_backup.txt' # filename for the backup
    ...
    def update_settings_file(self, fname, s):
        """Function to check if the existing fname (Cubotino_m_settings.txt) has all
        the parameters that were instroduced after the first release.
        Cubotino_m_settings.txt file will be updated with eventually missed parameters
    ...
    def update_servos_settings_file(self, fname, servo_s, json):
        """Function to check if the existing setting file(fname: Cubotino_m_servos_settings.txt)
            has all the parameters that were introduced after first release.
            Cubotino_m_servos_settings.txt will be eventually updated with missed parameters
            and related default values.
            """
...
if __name__ == "__main__":
    ...
    print()
    print("#"*80)
    print("This script is only meant to be imported by others Cubotino scripts")
    print("When executed, it just loads and prints the settings from the JSON file as test")
    print("#"*80)
    print()
